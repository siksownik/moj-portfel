...
app_mode = st.radio(
    label="Choose mode:",
    options=["Optimization (Auto)", "Custom portfolio with predetermined weights (Manual)"],
    horizontal=True,
...
with st.expander("📝 How to determine these values? (Instructions)", expanded=True):
    st.markdown("""
    **Part A: Vanguard Investor Questionnaire** Select your 'A' value based on your recommended stock allocation:
...
if tickers_input:
    if app_mode == "Custom portfolio with predetermined weights (Manual)":
...
if st.button("🚀 GENERATE FULL REPORT"):
    ...
    if all_data:
        ...
        if var_method == "Parametric (Normal Dist)":
            ...
            # Scaling daily mean and vol to the chosen horizon
            ...
            # Scale historical daily returns to the horizon
